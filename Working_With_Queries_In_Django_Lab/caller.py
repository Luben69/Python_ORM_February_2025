...
def find_books_by_genre_and_language(wanted_genre, wanted_language):
    return Book.objects.filter(genre=wanted_genre, language=wanted_language)


def find_authors_nationalities():
    all_authors = Author.objects.exclude(nationality=None)

    return '\n'.join(f"{a.first_name} {a.last_name} is {a.nationality}" for a in all_authors)


def order_books_by_year():
    wanted_books = Book.objects.all().order_by('publication_year', 'title')

    return '\n'.join(f"{w.publication_year} year: {w.title} by {w.author}" for w in wanted_books)
...
def filter_authors_by_nationalities(given_nationality):
    wanted_authors = Author.objects.filter(nationality=given_nationality).order_by('first_name', 'last_name')

    return '\n'.join(w.biography if w.biography is not None else f"{w.first_name} {w.last_name}" for w in wanted_authors)


def filter_authors_by_birth_year(start_year, end_year) -> str:
    authors = Author.objects.filter(
        birth_date__gte=date(start_year,1,1),
        birth_date__lte=date(end_year, 12, 31)
    ).order_by('-birth_date')
